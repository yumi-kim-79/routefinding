...
| **Android 앱 ID / 배너 단위 ID** | ❌ **아직 테스트 ID** — §3에서 교체 |

**iOS**는 release 빌드에서 실제 광고가 나간다.
**Android**는 아직 구글 공식 테스트 광고("Test Ad" 라벨)가 나온다 — 동작은 정상, 수익만 0이다.

적용된 iOS 값:
- 앱 ID `ca-app-pub-4653853586463291~2632257524` → `app/app.json`
- 배너 단위 `ca-app-pub-4653853586463291/6926725756` → `app/src/constants/ads.ts`

⚠️ 디버그 빌드(`__DEV__`)에서는 **iOS도 테스트 광고**가 나온다(§3-3). 정상이다.
