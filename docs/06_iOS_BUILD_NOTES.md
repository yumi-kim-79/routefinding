...
### 🕳️ iOS 함정 — 크기를 재기 전에 `<Image>`를 올리면 저해상도로 캐시된다 (2026-08-05)
증상: 사진을 처음 첨부하면 **모자이크처럼 뭉개져** 보이고, 나갔다 다시 첨부하면 정상.
매번 같은 패턴으로 재현된다.

원인: 컨테이너를 `onLayout`으로 재기 전 첫 렌더에서는 크기가 **1×1**이다.
그때 `<Image>`가 마운트되면 **iOS가 1px로 디코딩한 비트맵을 캐시**하고,
뷰가 커진 뒤에도 그것을 확대해 쓴다.

→ **크기를 잰 뒤에 이미지를 그린다.** `key`에 크기를 넣어 크기 변경 시 다시 디코딩하게 한다.
```tsx
{box.w > 1 ? <Image key={`${uri}:${box.w}x${box.h}`} ... /> : null}
```

> ⚠️ 처음엔 `react-native-image-picker`의 `maxWidth`가 저해상도 임시본을 준다고 의심했는데
> **오진이었다.** 옵션을 빼도 증상이 그대로였다. 같은 함정을 다시 밟지 않도록 남겨둔다.
