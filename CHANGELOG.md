...
#### ✅ 서명 — Play 앱 서명 **켜져 있음** (확인 완료)
인증서가 앱 서명 키 / 업로드 키로 분리돼 있다 → 업로드 키를 잃어도 **재설정 요청으로 복구 가능**.
남은 일은 로컬 `.jks` 를 찾아 SHA-1 `C4:29:26:ED:…` 와 대조하는 것뿐이다.

#### 🚨 새로 발견 — **Play 앱 서명은 앱의 지문을 바꾼다**
업로드한 AAB는 Google이 **앱 서명 키로 다시 서명**해서 배포한다.
즉 사용자 기기의 SHA-1은 우리 로컬 키가 아니라 `3C:79:9A:EC:…:6C:9D` 다.
지문으로 앱을 식별하는 곳에 이 값을 등록하지 않으면 **스토어 버전에서만 조용히 깨진다**:
- **Google Maps API 키** 제한 → 빠뜨리면 스토어 버전에서 **지도가 회색**
- **Firebase 디지털 지문**(SHA-1 + SHA-256) → 빠뜨리면 **App Check 토큰 실패 → Firestore 접근 차단**

빌드로는 절대 안 잡히고 **올린 뒤에야 드러나는** 종류다. → 내부 테스트 트랙 선행 권장.
자세한 절차는 docs/08 §2-5-1.
...
#### 실측 — ABI 축소는 먹었다
`app-release.apk` **35,418,705 bytes (33.8MiB)**, `lib/`은 arm64-v8a 14.7MB +
armeabi-v7a 10.5MB **두 개뿐**. x86 계열 30.5MB가 실제로 빠졌다(24차 수정 검증 완료).
> 기기 설정에 뜨는 설치 용량은 APK보다 크다 — 설치 시 dex가 최적화(.odex/.vdex)되기 때문이다.

#### Fixed — `height: 0`으로 눌러 담던 것을 없앰
로드 전 배너를 `height: 0 / overflow: hidden`으로 강제로 접고 있었다.
**적응형 배너는 자기 너비를 재서 광고를 요청**하는데 부모가 0이면 요청이 어그러질 수 있다.
`BannerAd`는 로드 전에 아무것도 그리지 않아 **가만히 둬도 높이가 0**이다 — 누를 필요가 없었다.
테두리·배경도 로드 후에만 준다(빈 화면에 실선 한 줄이 남던 문제).

#### Fixed — 실패 이유를 삼키지 않는다
`onAdFailedToLoad`를 통째로 삼켜서 **연동이 틀린 건지 재고가 없는 건지 구분할 수 없었다.**
이제 `console.warn('[ads] 배너 로드 실패:', unitId, error)`를 남긴다.
확인: `adb logcat | grep '\[ads\]'` / Xcode 콘솔.

#### 🔧 임시 — `FORCE_TEST_ADS = true`
배너가 안 보이는 원인은 둘인데 화면만 봐서는 구분이 안 된다:
1. 연동이 잘못됨 (ID·초기화·레이아웃)
2. 연동은 맞는데 **신규 광고 단위라 아직 재고가 안 붙음**
   — AdMob 신규 단위는 첫 노출까지 보통 몇 시간, 길면 24시간.
     iOS 앱은 아직 App Store에 없어 더 적게 채워진다.

구글 테스트 광고는 **항상 100% 채워진다.** 한 번 띄워 보면 1인지 2인지 바로 갈린다.
→ `constants/ads.ts`의 `FORCE_TEST_ADS`를 **확인 후 반드시 `false`로 되돌릴 것.**
...
> 무엇을 눌러야 할지 헷갈렸다(사용자 지적). 사진에 라인을 긋는 일은 결국 **제보의 일부**다 —
> 개념도란 결국 '선이 그려진 사진'이니까. 그래서 **사진을 첨부하는 자리**로 옮겼다.

#### Added — 편집기의 **로컬 편집 모드**
- `ConceptPhotoEditor`에 `onPicked` / `initialUri` 추가
  - `onPicked`가 있으면 **업로드하지 않는다.** 합성된 로컬 사진 uri를 돌려주고 닫는다
    → 제보를 저장할 때 다른 첨부 사진과 **함께** 올라간다
  - `initialUri`로 **이미 첨부한 사진을 열어** 바로 그릴 수 있다
  - 저장 버튼 문구도 모드에 따라 바뀐다: `등록 (승인 요청)` ↔ `이 사진 사용`
  - 합성 실패(구형 빌드에 view-shot 없음) 시에도 로컬 모드는 '원본 그대로 첨부'를 제안한다

#### Added — 루트제보 폼
- 사진 첨부 아래에 **'사진 등록 · 라인 그리기'** 버튼 (촬영/앨범 → 선·글자 → 첨부)
- 첨부한 썸네일마다 **연필 버튼** — 그 사진에 라인을 그린다(결과로 교체)
- `useReportForm`: `addImageUri` / `replaceImageUri`
  (합성 결과는 새 로컬 파일이라 `remoteUrl`을 비워야 업로드 대상이 된다)

#### Removed
- **개념도 상세**의 '사진 등록 · 라인 그리기' 버튼 (중복 · 혼란)
  - 기존 개념도에 사진만 더하는 흐름은 **개념도 목록 카드의 카메라 버튼**에 그대로 있다

#### 검증
- `tsc --noEmit` 에러 0 / `eslint` 에러 0


### 📅 2026-08-05 (20차) — 보던 개념도에서 바로 루트 제보 · 관리자 버튼 정리

#### Changed — 사진 승인 버튼을 **승인 / 반려 / 삭제** 셋으로 (사용자 결정)
- 예전엔 관리자 카드에 **'승인·추가'와 '승인·교체'** 두 개가 나란히 있었다.
  '교체'는 **개념도의 기존 사진을 전부 지우고** 이 사진 하나만 남기는 동작이라 오조작 위험이 컸다.
- 승인은 이제 항상 **추가**(`'add'`)로만 동작한다 — 되돌릴 수 있는 쪽이다.
  사진 정리가 필요하면 개념도 수정 화면에서 지운다.
- 서비스의 `replace` 모드 자체는 남겨뒀다(옛 문서의 `applyMode` 값과 호환).

#### Added — 개념도를 보다가 그 자리에서 루트 제보
> 같은 바위에 루트를 하나 더 올리려면 **홈으로 나가 루트제보 탭을 열고 등반지·구역·좌표를
> 처음부터 다시 골라야 했다.** 보고 있던 화면에서 열면 그 값들이 이미 채워져 있다.

- `ReportPrefill` 타입 + `useReportForm(edit, prefill)` — 빈 값은 무시하고 있는 값만 채운다
- `ReportWriteRoute` — 탭 화면(`ReportWriteScreen`)을 **스택에서도** 열기 위한 얇은 껍데기.
  화면 본체를 건드리지 않아 탭 동작은 그대로다
- **개념도 상세**: "이 구역에 루트 제보" 버튼 — 등반지·구역·좌표·구분이 채워진 채로 열린다
- **개념도 목록**: 검색 결과가 **한 등반지**로 좁혀졌을 때만
  "○○ · △△에 루트 제보" 버튼을 띄운다.
  여러 등반지가 섞인 결과에서는 무엇을 채울지 알 수 없으므로 **띄우지 않는다**
  (엉뚱한 등반지가 채워지는 것보다 안 띄우는 편이 낫다)
- 저장하면 보던 화면으로 돌아온다 (탭에서 연 경우는 그대로 남는다)

#### 검증
- `tsc --noEmit` 에러 0 / `eslint` 에러 0


### 📅 2026-08-05 (19차) — 🔍 제보 상세 화면 (관리자가 내용을 보고 승인할 수 있게)

> **제보관리에서 카드를 눌러도 빈 화면이 떴다.** `RouteDetail` 라우트가 아직
> `PlaceholderScreen`("후속 스프린트 구현 대상")으로 등록돼 있었기 때문이다.
> 목록 카드에는 썸네일 1장과 '산 · 루트명'뿐이라, **관리자가 내용을 못 보고 승인**하는
> 상태였다. 사진이 제대로 올라갔는지, 좌표가 찍혔는지, 피치가 몇 개인지 확인할 방법이 없었다.

#### Added — `screens/report/ReportDetailScreen.tsx`
- 상태(승인 대기/반려됨/…)와 **반려 사유**를 맨 위에 (검토 화면이라 상태가 먼저다)
- 사진 캐러셀 → 누르면 **핀치 줌 뷰어**(공용 `ConceptImageViewer` 재사용)
- 제보 내용 전체: 구분·등반지·구역·루트명·등반 형태·난이도·평균 난이도·길이·개척자·장비·찾아가는 길·번호·**좌표**·작성자
- **접근로 보기** — 18차에 만든 `ApproachMapModal` 재사용 (GPX/옛 기록)
- 피치 목록 (썸네일 누르면 뷰어로)
- 검토 액션은 **맨 아래** — 내용을 다 본 다음 누르게 한다: 승인 / 반려(사유 입력) / 삭제
  - 처리 후 `clearConceptCache()` + `goBack()` → 목록은 구독이 알아서 갱신
  - 컬렉션은 **문서에서 읽은 `source`**를 쓴다 (하드코딩하면 볼더링 제보에서 엉뚱한 문서를 건드린다)

#### Changed
- `MainNavigator`: `RouteDetail` / `ReportDetail` 플레이스홀더 → 실화면으로 교체
  (옛 라우트명 `ReportDetail`로 들어오는 경로도 같은 화면을 보게 남겨뒀다)
- `MainStackParamList`: `RouteDetail`에 **`collection?`** 추가.
  목록에서 넘어올 땐 어느 컬렉션인지 알고 있으니 알려준다 — 모르면 두 곳을 순서대로 시도
- `ReportCard`: 이동할 때 `collection`을 함께 넘긴다

#### 검증
- `tsc --noEmit` 에러 0 / `eslint` 에러 0 (인라인 스타일 경고만)


### 📅 2026-08-05 (18차) — 🗺️ 첨부한 GPX를 실제로 볼 수 있게 (접근로 보기)

> v2에서 **GPS 실시간 따라가기를 없앤 대신** 접근로는 GPX 첨부로 남기기로 했는데,
> 정작 앱에는 **그 파일을 보는 화면이 없었다.** 개념도 상세에 `접근로 GPX: 등록됨`
> 글자만 떴다. 첨부만 되고 열 수 없으면 첨부의 의미가 없다.

#### Added
- `services/gpxService.ts` — GPX 파싱·조회
  - `<trkpt>` → 없으면 `<rtept>` → 없으면 `<wpt>` 순으로 좌표를 찾는다 (산에서 만든 파일은 종종 불완전하다)
  - **XML 파서 라이브러리를 넣지 않았다.** 쓰는 건 lat/lon 속성뿐이라 정규식이 더 가볍고 깨진 파일에 관대하다
  - 같은 파일을 다시 열 때 재다운로드하지 않도록 앱 수명 캐시
  - 하버사인으로 총 거리 계산 (`1.23km` / `450m` 표기)
- `screens/route/components/ApproachMapModal.tsx` — 접근로 지도
  - `fitToCoordinates`로 트랙 전체가 한 화면에 들어오게 맞춘다 (헤더·요약바 높이만큼 여백)
  - **흰 선(8px) 위에 색 선(4px)** 두 겹 — 한 겹만 그리면 위성/등고선 배경에 묻힌다
  - 출발(초록)·도착(빨강)·루트 위치 마커, 총 거리와 지점 수 표시
  - 지도탭과 **같은 provider(구글)** — 화면마다 다른 지도가 나오면 혼란스럽다
- `ConceptDetailScreen`에 **"접근로 보기"** 버튼 (GPX가 있을 때만 노출)

#### Fixed
- **GPX도 이미지와 같은 403 함정에 걸린다** — DB에 `storage.googleapis.com/...` 원본 주소로
  저장된 문서는 Storage 규칙이 아니라 GCS IAM을 타서 그냥 `fetch`하면 403이다.
  `resolveImageUrl()`을 태워 다운로드 URL로 바꾼 뒤 받는다 (이미지에서 쓰던 그 처리)
- 루트 좌표 마커: `Number('')`는 **0**이라 그대로 쓰면 좌표 없는 루트가 (0,0) 아프리카 앞바다에 찍힌다 → 빈 값·(0,0) 제외

#### Added — 타입
- `Concept.trackingPath` — v1 시절 실시간 기록 배열. 기록 기능은 없앴지만 **옛 문서에는 남아 있어**
  GPX가 없을 때 **읽기 전용 폴백**으로 그린다 (웹 `ReportView.vue`도 필드를 지우지 않고 빈 배열로만 저장한다)

#### 검증
- `tsc --noEmit` 에러 0 / `eslint` 에러 0 (인라인 스타일 경고만, 기존 코드와 동일 수준)
